...
namespace planner
{
  ...
  void ReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    if (planner_manager_->pp_.drone_id >= 1)
    {
      ...
      swarm_trajs_sub_ = nh.subscribe(sub_topic_name.c_str(), 10, &ReplanFSM::swarmTrajsCallback, this, ros::TransportHints().tcpNoDelay());
    }
    ...
    broadcast_bspline_sub_ = nh.subscribe("planning/broadcast_bspline_to_planner", 100, &ReplanFSM::BroadcastBsplineCallback, this, ros::TransportHints().tcpNoDelay());
  ...
  void ReplanFSM::BroadcastBsplineCallback(const traj_utils::BsplinePtr &msg)
  {
    ...
    planner_manager_->swarm_trajs_buf_[id].start_pos_ = pos_traj.evaluateDeBoorT(0);
  ...
  void ReplanFSM::swarmTrajsCallback(const traj_utils::MultiBsplinesPtr &msg)
  {
    ...
    for (size_t i = 0; i < msg->traj.size(); i++)
    {
      ...
      planner_manager_->swarm_trajs_buf_[i].start_pos_ = pos_traj.evaluateDeBoorT(0);
